  ...
  <aside>
    ...
    <div className={`${style.name} centerAlign`}>
      <div className={`${style.boxName} centerAlign`}>
        <h2>
          {Config.authorInfo.name.split(' ')[0]}
          {' '}
          <span>{Config.authorInfo.name.split(' ')[1]}</span>
        </h2>
      </div>
      <div className={`${style.badge} ${style.badgeGray}`}>{Config.authorInfo.job}</div>
...
&nbsp; &nbsp;
          <a
            href={`mailto:${Config.authorInfo.email}`}
            target="_top"
          >
      ...
      <div className={style.resumeDownload}>
        <a href="docs/resume.pdf" download target="_blank">Download CV</a>
      </div>
    </div>
  </aside>
);
